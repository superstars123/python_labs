...
| Файл | Назначение |
|------|------------|
| `main.py` | Точка входа приложения. Загружает список игроков из JSON, запускает CLI-меню, при выходе сохраняет данные. |
| `cli.py` | Консольный интерфейс. Реализует меню, ввод пользователя, вывод информации и обработку ошибок ввода. |
| `app.py` | Бизнес-логика приложения. Управляет коллекцией игроков: добавление, удаление, поиск и сортировка. |
| `models.py (lab04)` | Содержит классы `Player`, `Enemy`, `PremiumPlayer` и логику расчёта силы персонажа (`calculate_power`). |
| `storage.py` | Модуль для сохранения и загрузки данных из JSON-файла (`players.json`). |
| `players.json` | Файл хранения данных игроков. Создаётся автоматически при первом сохранении. |
...
Запись консольного запуска (asciinema):
(https://asciinema.org/a/cs8kHlmv2WLa21P8)